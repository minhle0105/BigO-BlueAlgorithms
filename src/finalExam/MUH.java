    ...
    public static void main(String[] args) {
        ...
        if (ways >= 3l) {
            ...
            if (map.get(hasSeenKey).size() >= 3) {
                // Xử lí trường hợp tất cả các số khác xuất hiện 1
                // lần và chỉ có một số xuất hiện 3 lần trở lên
                for (Integer key : map.keySet()) {
                    if (key != hasSeenKey) {
                        for (Integer index : map.get(key)) {
                            options.get(i).add(index + 1);
                        ...
                        // Chuyển phần xử lý hasSeenKey vào đây để các key được
                        // thêm vào theo đúng thứ tự (nhỏ đến lớn)
                        ...
                        for(int j=3; j<map.get(hasSeenKey).size(); j++) {
                            options.get(i).add(map.get(hasSeenKey).get(j));
                        }
                    }
                }
            }
